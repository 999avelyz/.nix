...
def generate_config_entry(
    ...
    depth = base_depth
    entry = ""
    display_label = label if label is not None else f"Generation {gen}"
    ...
    if boot_spec.xen and "version" in boot_spec.xen:
        ...
        if config("efiSupport"):
            entry += xen_config_entry(
                base_depth, boot_spec, xen_version, gen, time, True
            )
        entry += xen_config_entry(base_depth, boot_spec, xen_version, gen, time, False)
    ...
    if len(specialisation_list) > 0:
        ...
        entry += config_entry(depth, boot_spec, display_label, str(time))
...
def install_bootloader() -> None:
    ...
    system_gens = sorted(get_gens(), key=lambda x: x, reverse=True)
    last_gen = system_gens[0]
    old_gens = system_gens[1:]
    ...
    config_file += textwrap.dedent(f"""
        ...
        graphics: yes
        default_entry: {2 if len(last_gen_boot_spec.specialisations.items()) > 0 else 1}
    """)
    ...
    # The latest system generation is exposed as a standalone top-level
    # entry (rather than nested inside the generations directory), so it
    # sits alongside "Windows 11" as its own menu item.
    config_file += generate_config_entry(
        "system", last_gen, True, base_depth=1, label=config("distroName")
    )

    # extraEntries (e.g. Windows 11) is placed right after the latest
    # NixOS generation, before the older-generations directory.
    config_file += str(config("extraEntries"))

    if old_gens:
        config_file += "/NixOS Old Generations\n"

        isFirst = True

        for gen in old_gens:
            config_file += generate_config_entry("system", gen, isFirst)
            isFirst = False

    for profile in get_profiles():
        gens = get_gens(profile)
        config_file += f"/+{config('distroName')} profile '{profile}'\n"
